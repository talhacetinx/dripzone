...
// Feature toggle: if you ever want S3 support enable via env var
...
export const saveFile = async (fileData, fileType, subFolder = '') => {
  try {
    ...
    if (USE_S3) {
      // Upload to S3 and return full URL
      const s3Url = await uploadToS3(base64Data, mimeType, fileType, subFolder, fileName);
      return {
        success: true,
        filePath: s3Url,
        fileName,
    ...
    // Save runtime uploads into project's `uploads` folder (served by app/api/uploads/[...path])
    const cleanSub = subFolder ? String(subFolder).replace(/^\/+|\/+$/g, '') : '';
    const uploadDir = path.join(process.cwd(), 'uploads', fileType, cleanSub);
  ...
  // Return API-served URL so all runtime files are fetched via /api/uploads/...
  const publicPath = `/api/uploads/${fileType}/${cleanSub ? cleanSub + '/' : ''}${fileName}`;
...
export const deleteFile = (filePath) => {
  try {
  // Ensure we join relative paths correctly. If filePath starts with '/', strip it.
    let relPath = typeof filePath === 'string' ? filePath : '';
    // Normalize known public URL prefixes to the runtime uploads folder
    ...
    // Delete from the runtime uploads folder (not public)
