...
export async function GET() {
  try {
    ...
    // Mix Engineer serviceData'sını çek
    ...
    if (!providerProfile || providerProfile.serviceType !== 'mix_engineer') {
      return NextResponse.json({
        error: "Mix Engineer profili bulunamadı"
      }, { status: 404 });
    }
    ...
    return NextResponse.json({
      serviceType: 'mix_engineer',
      serviceData: serviceData || {} // Direkt serviceData'yı döndür
    });

  } catch (error) {
    console.error("Mix Engineer service data error:", error);
    return NextResponse.json(
      { error: "Sunucu hatası" },
      { status: 500 }
    );
