...
export async function GET() {
  try {
    ...
    if (!providerProfile || (providerProfile.serviceType !== 'producer' && providerProfile.serviceType !== 'music_producer')) {
      return NextResponse.json({
        error: "Producer profili bulunamadı"
      }, { status: 404 });
    }
    ...
    console.error("Producer service data error:", error);
    return NextResponse.json(
      { error: "Sunucu hatası" },
      { status: 500 }
    );
